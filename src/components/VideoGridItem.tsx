  ...
  return (
    ...
    >
      <a href={`/watch?v=${id}`} className="relative aspect-video">
        <img
          src={thumbnailUrl}
          className={`block size-full object-cover transition-[border-radius] duration-200 ${isVideoPlaying ? 'rounded-none delay-200' : 'rounded-xl'}`}
        />
      ...
      <div className="flex gap-2">
        ...
        <div className="flex flex-col">
          ...
          <div className="text-secondary-text text-sm">
            {VIEW_FORMATTER.format(views)} View • {moment(postedAt).fromNow()}
          </div>
        </div>
      </div>
    </div>
